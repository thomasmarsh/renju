...
type BitBoardT w h a = (KnownNat w, KnownNat h, Bits a, Integral a)

instance BitBoardT w h a => Bits (BitBoard w h a) where
    (.&.)                 = applyBin (.&.)
    (.|.)                 = applyBin (.|.)
    xor                   = applyBin xor
    ...
    isSigned (BitBoard m) = isSigned m
    testBit (BitBoard m)  = testBit m
    popCount (BitBoard m) = popCount m
    bit i                 = BitBoard (bit i)

instance BitBoardT w h a => FiniteBits (BitBoard w h a) where
...
erode :: BitBoardT w h a
      => Direction -> BitBoard w h a -> BitBoard w h a
erode dir b = b .&. shiftD dir b

{-
dilate :: BitBoardT w h a
       => Direction -> BitBoard w h a -> BitBoard w h a
...
-- or diagonal.
hasNInRow :: BitBoardT w h a => BitBoard w h a -> Int -> Bool
